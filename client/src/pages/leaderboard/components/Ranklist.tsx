  ...
  return (
    <div className="cursor-pointer w-full h-40 border-[#A88DEB] border-2 rounded-2xl flex flex-row p-5 mb-5 items-center justify-bewteen" onClick={viewPort}>
      ...
      <div className="flex items-center">
        ...
        {totalpl >= 200000 ? (
          <p className="font-bold text-4xl text-green-600 px-12 w-[270px] overflow-hidden">+{totalpl.toLocaleString(undefined, { maximumFractionDigits: 2 })}</p>
        ) : (
          <p className="font-bold text-4xl text-red-500 px-12 w-[270px] overflow-hidden">{totalpl.toLocaleString(undefined, { maximumFractionDigits: 2 })}</p>
        )}
      </div>
    </div>
  );
